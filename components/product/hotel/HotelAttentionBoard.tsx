...
/**
 * Hotel Needs attention — DEMO fixtures, not Berlin restaurant findings.
 */
export function HotelAttentionBoard() {
  ...
  return (
    <div className="rp-attention rp-hotel-attention" aria-label="Hotel needs attention">
      <header className="rp-attention-head">
        <p className="rp-hotel-glance-kicker">CANAL HOUSE · DEMO</p>
        <h1 className="rp-attention-title">Needs attention</h1>
        <p className="rp-attention-sub">
          {open.length === 0
            ? "Nothing needs you. RADR is handling the rest."
      ...
      {open.length === 0 ? (
        <p className="rp-hotel-glance-lead">
          <Link href="/app/service">Open property board</Link>
          {" · "}
          ...
          {open.map((item, i) => (
            <li key={item.id} className="rp-hotel-attn-row">
              ...
              <div className="rp-hotel-attn-main">
                ...
                <p className="rp-hotel-attn-need">
                  WHAT RADR NEEDS FROM YOU · one approval
                </p>
              </div>
              ...
              <div className="rp-hotel-attn-cta">
                <Link href={item.href} className="rp-btn-secondary">
                  {item.cta}
                </Link>
                <button
                  type="button"
